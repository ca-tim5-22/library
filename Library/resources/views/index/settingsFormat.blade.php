                    ...
                    <table class="overflow-hidden shadow-lg rounded-xl min-w-full border-[1px] border-[#e4dfdf]" id="myTable">
                        <thead class="bg-[#EFF3F6]">
                            <tr class="border-b-[1px] border-[#e4dfdf]">
                                ...
                                <th id="default_checked" class="px-4 py-4 leading-4 tracking-wider text-left">Naziv formata
<a 
                                ...
                                <th id="if_checked" class="none px-4 py-4 text-sm leading-4 tracking-wider text-left">
                                    <form id="format_delete_all" action="" method="POST">
                                        @csrf
                                        @method("get")
                                ...
                                <th style="color: rgb(58, 26, 152);font-weight:600;font-style:italic;" id="if_one_checked" class="none px-4 py-4 text-sm leading-4 tracking-wider text-left">
                                    <a id="format_edit" href="">
                                    Izmijeni format</a>
                                </th>
                                <th style="color: rgb(58, 26, 152);font-weight:600;font-style:italic;" id="if_one_checked" class="none px-4 py-4 text-sm leading-4 tracking-wider text-left">
                                    <form id="format_delete" method="POST" action="">
                                        @csrf
                                        @method('DELETE')
                                        <button style="color: rgb(58, 26, 152);font-weight:600;font-style:italic;" type="submit" name="submit">
                                            Izbrišite format
                        ...
                        <tbody class="bg-white">
                            ...
                            <tr class="hover:bg-gray-200 hover:shadow-md border-b-[1px] border-[#e4dfdf]">
                                <td class="px-4 py-4 whitespace-no-wrap">
                                    <label class="inline-flex items-center">
                                        <input id="table_checkboxes" type="checkbox" class="form-checkbox" data-format-id="{{$f->id}}">
                                    </label>
                                </td>
                                    ...
                                    <div style="position:absolute;right:80px;" class="z-10 hidden transition-all duration-300 origin-top-right transform scale-95 -translate-y-2 dropdown-format">
                                        <div class="absolute right-[25px] w-56 mt-[7px] origin-top-right bg-white border border-gray-200 divide-y divide-gray-100 rounded-md shadow-lg outline-none" aria-labelledby="headlessui-menu-button-1" id="headlessui-menu-items-117" role="menu">
                                            <div class="py-1">
                                                ...
                                                <form action="{{route('format.destroy',$f->id)}}" method="POST"class="flex w-full px-4 py-2 text-sm leading-5 text-left text-gray-700 outline-none hover:text-blue-600"
                                                    role="menuitem" >
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit">
                    ...
                    <div class="flex flex-row items-center justify-end my-3">
                        ...
                        <div> 
                        @if (URL::current() == "http://127.0.0.1:8000/formatsort")
                             <form style="margin-right:20px;" id="formica" method="POST" action="{{route("format.sort")}}">
                             @else
                            <form style="margin-right:20px;" id="formica" method="POST" action="{{route("format.index")}}">
                        @endif
                        ...
                        @method("GET")
                         <select onchange="this.form.submit()" id="selectica" form="formica" 
                            ...
                            <p class="inline text-md">
                              ...
                              {{ $formats->onEachSide($currentpag)->links("vendor\pagination.tailwind") }}
                            </p>
        ...
        checkboxes.forEach(e => {
            e.addEventListener("change",()=>{
                ...
                if(nmb_of_checked == 1){
                    ...
                    var format_edit = document.getElementById("format_edit");
                    var format_delete = document.getElementById("format_delete");
        
                    var id = e.dataset.formatId;
                    
                   
                   
                    format_edit.setAttribute("href","http://127.0.0.1:8000/format/"+id+"/edit");
                    
                    
                    format_delete.setAttribute("action","http://127.0.0.1:8000/format/"+id);
                    ...
                    var format_delete_more = document.getElementById("format_delete_all");
                    var checked = document.querySelectorAll("#table_checkboxes:checked");
                    var ids="";
                    checked.forEach(checked =>{
                        ids += "-"+checked.dataset.formatId;
                    })
                    
                    ids = ids.slice(1);
                    format_delete_more.setAttribute("action","http://127.0.0.1:8000/deleteformat/"+ids)
                    default_checked.forEach(l =>{
                        l.classList.add("none");
                    });
                    if_one_checked.forEach(o =>{
